  ...
  const allLeagues = Array.from(
    new Set(
      processedTeams.flatMap((team) =>
        team.players ? team.players.map((p) => p.league).filter(Boolean) : [],
      ),
    ),
  );

  const [allYearsOne, setAllYearsOne] = useState({ numbers: [] });
    const [allYearsTwo, setAllYearsTwo] = useState({ numbers: [] });
    const [leaguesOne, setLeaguesOne] = useState([ leagues: [] ]);
    const [leaguesTwo, setLeaguesTwo] = useState([ leagues: [] ]);

    useEffect(() => {
        const yearsEndpointOne = `${API_BASE_URL}${ENDPOINTS.TEAM}/years/${processedTeams[0].name}/${allLeagues[0]}`;
        const yearsEndpointTwo = `${API_BASE_URL}${ENDPOINTS.TEAM}/years/${processedTeams[1].name}/${allLeagues[1]}`;

        const leaguesEndpointOne = `${API_BASE_URL}${ENDPOINTS.TEAM}/leagues/${processedTeams[0].name}`;
        const leaguesEndpointTwo = `${API_BASE_URL}${ENDPOINTS.TEAM}/leagues/${processedTeams[1].name}`;

        if (teamOne) {
            fetch(yearsEndpointOne)
                .then((respone) => respone.json())
                .then((data) => setAllYearsOne(data))
                .catch((error) => console.error("Error fetching years:", error));

            fetch(leaguesEndpointOne).then((response) => response.json()).then((data) => setLeaguesOne(data)).catch((error) => console.error("Error fetching league one:", error));
        }
        if (teamTwo) {
            fetch(yearsEndpointTwo)
                .then((response) => response.json())
                .then((data) => setAllYearsTwo(data))
                .catch((error) => console.error("Error fetching years:", error));

            fetch(leaguesEndpointTwo).then((response) => response.json()).then((data) => setLeaguesTwo(data)).catch((error) => console.error("Error fetching league two:", error));
        }

    }, []);

  // State for selected year and league
  const [selectedYearOne, setSelectedYearOne] = React.useState("all");
  const [selectedLeagueTwo, setSelectedLeagueTwo] = React.useState("all");
  ...
  const filteredTeams = processedTeams.map((team) => ({
    ...
    data: team.players
      ? team.players
          .filter(
            (p) =>
              (selectedYearOne === "all" || p.year === selectedYearOne) &&
              (selectedLeagueOne === "all" || p.league === selectedLeagueOne),
          )
          .map((p) => p.rankingValue)
          .filter((val) => val != null)
      : [],
    ...
    scales: {
      y: {
        ...
        min:
          globalMin === globalMax
            ? globalMin - 1
            : globalMin - Math.abs(globalMin * 0.1),
        max:
          globalMin === globalMax
            ? globalMax + 1
            : globalMax + Math.abs(globalMax * 0.1),
        title: {
          display: true,
          text: "Ranking Value",
        },
