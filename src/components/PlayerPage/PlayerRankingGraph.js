...
const PlayerRankingGraph = ({ data }) => {
  ...
  const chartData = useMemo(() => {
    ...
    return data
      .filter(item => item[valueField] !== null && item[valueField] !== undefined)
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="custom-tooltip">
          ...
          <p><strong>Ranking Value:</strong> {tooltipData.rankingValue.toFixed(2)}</p>
          <p><strong>Team:</strong> {tooltipData.team}</p>
          <p><strong>League:</strong> {tooltipData.league}</p>
        </div>
      );
    }
    return null;
  ...
  if (chartData.length <= 3) {
    return (
      <div className="insufficient-data-message">
        Chart requires more than 3 data points to display trends effectively. Currently showing {chartData.length} data point{chartData.length === 1 ? '' : 's'}.
      </div>
    );
  }
  ...
  return (
    <div className="PlayerRankingGraph">
      ...
      <h3 className="graph-title">
        {showRankingValue ? 'Ranking Value Over Time' : 'Display Value Over Time'}
      </h3>
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          <Line
            ...
            activeDot={{ r: 6 }}
            name={showRankingValue ? "Ranking Value" : "Display Value"}
          />
        </LineChart>
      </ResponsiveContainer>
    </div>
